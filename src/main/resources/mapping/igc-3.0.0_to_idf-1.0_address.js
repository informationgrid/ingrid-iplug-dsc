...
function getIdfResponsibleParty(addressRow, role, specialElementName) {
    ...
    var rows = SQL.all("SELECT DISTINCT t01_object.* FROM t01_object, t012_obj_adr WHERE t012_obj_adr.adr_uuid=? AND t012_obj_adr.obj_id=t01_object.id AND t01_object.work_state=? AND t01_object.publish_id=?", [addressRow.get("adr_uuid"), 'V', '1']);
    for (var j=0; j<rows.size(); j++) {
